CREATE OR REPLACE PROCEDURE eemrt.sp_Add_Contract_PRISM_old(
...
IS
  ...
  v_contract_NUM Contract.Contract_Number%TYPE;
...
END IF;
     ...
     SP_INSERT_AUDIT(p_CREATED_BY,  'sp_Add_Contract_PRISM   '||LENGTH(v_contract_NUM)||' Char, '||p_CONTRACT_NUMBER||'\'||'p_DO_NUM='||p_DO_NUM );      
...
--p_CONTRACT_NUMBER  := RTRIM(LTRIM(p_CONTRACT_NUMBER)) ||'/'|| RTRIM(LTRIM(vDO_NUM));
  ...
  BEGIN
   ...
   /* IF LENGTH(p_CONTRACT_NUMBER) < 17 THEN
      ...
      RETURN ;
    END IF;*/
     SP_INSERT_AUDIT(p_CREATED_BY,  'sp_Add_Contract_PRISM   Contract Create attempted, '||p_CONTRACT_NUMBER||'\'||'p_DO_NUM='||p_DO_NUM );     
    IF LENGTH(v_contract_NUM) = 17 THEN
     SP_INSERT_AUDIT(p_CREATED_BY,  'sp_Add_Contract_PRISM   17 Char, '||v_contract_NUM||'\'||'p_DO_NUM='||p_DO_NUM );  
...
/*    elsIF LENGTH(p_CONTRACT_NUMBER) < 22 THEN
    ...
    IF vContract_count = 0 THEN
     ...
     SP_SEND_EMAIL( SENDER => vSENDER, RECEIVER => vRECEIVER_usr, SUBJECT => vSUBJECT, MESSAGE => p_PStatus );           
      vMESSAGE  := 'A new contract '||v_contract_NUM||' for user '||p_CREATED_BY||' was attempted to put into eCert system.  Please review and verify the contract (if required)';
...
INSERT
  ...
  IF SQL%FOUND THEN
    ...
    vMESSAGE  := 'A new contract '||p_CONTRACT_NUMBER||' for user '||p_CREATED_BY||' in '||vROLE||' has been loaded in the system.  Please review and verify the COR Designation Letter (if required)';
    -- p_PStatus := 'The data for Contract '||TO_CHAR(p_CONTRACT_NUMBER)||' is being retrieved from PRISM and DELPHI.  You will be notified via email once data retrieval has been completed.  If you have not received an email within 24 hours, please notify the System Administrator';
    COMMIT;
  END IF;
  SP_SEND_EMAIL( SENDER => vSENDER, RECEIVER => vRECEIVER, SUBJECT => vSUBJECT, MESSAGE => vMESSAGE );
