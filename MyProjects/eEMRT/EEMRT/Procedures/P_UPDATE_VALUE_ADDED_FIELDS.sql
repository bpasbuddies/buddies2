CREATE OR REPLACE PROCEDURE eemrt.p_update_value_Added_fields
AS
BEGIN
   -- UPDATE Fund_type
  /* -- ARRA ( 6th and 7th charactors should be 'AS')
   ...
   --GRANTS FUND 4th and 5th charactors should be '81')
   UPDATE   delphi_contract_stage
   ...
   --PCB&T FUND 4th and 5th 6th charactors should be '82W')
   UPDATE   delphi_contract_stage
      SET   FUND_TYPE = 'PCB&T'
    WHERE       SUBSTR (FUND, 4, 3) = ('82W')
            AND SUBSTR (FUND, 6, 2) <> ('AS')
            AND SUBSTR (FUND, 3, 1) <> 'X'
            AND SUBSTR (FUND, 6, 1) = ('W');
   ...
   --REIMB FUND 4th,5th & 6th charactors should be '82R')
   UPDATE   delphi_contract_stage
      SET   FUND_TYPE = 'REIMB'
    WHERE       SUBSTR (FUND, 4, 3) = ('82R')
            AND SUBSTR (FUND, 6, 2) <> ('AS')
            AND SUBSTR (FUND, 3, 1) <> 'X';


   --FRANCH FUND 3rd charactor should be 'X')
   UPDATE   delphi_contract_stage
      SET   FUND_TYPE = 'FRANCH'
    WHERE   SUBSTR (FUND, 3, 1) = ('X') AND SUBSTR (FUND, 6, 2) <> ('AS');

   -- This fund is created only for SANDY hurricane
   UPDATE   delphi_contract_stage
      SET   FUND_TYPE = 'F&E-SANDY'
    WHERE   fund = '12X82AS000';
    */
